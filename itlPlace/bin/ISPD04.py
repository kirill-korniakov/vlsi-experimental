...
def parseLog(logName, benchmark):
    fh = open(logName, "r")
    po = open('pythonOutput_ispd04.xls', 'a')
...
po = open('pythonOutput_ispd04.xls', 'w')
#if (po == NULL):
...
for benchmark in testSet:
    logFileName = ".\\ISPD04\\" + benchmark + ".log"
    fPlacerOutput = open(logFileName, 'w');
    subprocess.Popen(["itlPlaceRelease.exe", cfgName,
        "--benchmark.def=.\\ISPD04\\" + benchmark + ".def",
        "--benchmark.lef=.\\ISPD04\\" + benchmark + ".lef"],
