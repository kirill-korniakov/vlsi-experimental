										...
										si2drErrorT   *err)
{
	...
	liberty_hash_enter_oid(g->attr_hash, attr->name, retoid);  /* fix via chrisj's posting: attr->name instead of name */
...
/* many thanks to Stefan Zager for reporting that AddExprValue was not available.
   (it is now!). */
...
si2drValueTypeT  si2drComplexValGetValueType( si2drAttrComplexValIdT attr,
											  si2drErrorT     *err )
{
	liberty_attribute_value *z = (liberty_attribute_value*)attr.v2;
	*err = SI2DR_NO_ERROR;
	if( !z )
	{
		return SI2DR_UNDEFINED_VALUETYPE;
    ...
    if(z->type != LIBERTY__VAL_INT)
	{
		*err = SI2DR_INVALID_OBJECTTYPE;
		if( liberty___debug_mode )
		{
			...
			(*si2ErrMsg)(SI2DR_SEVERITY_ERR, SI2DR_INVALID_OBJECTTYPE, 
						 "si2drComplexValGetInt32Value:", &err2);
		}
		return 0;
	}
	*err = SI2DR_NO_ERROR;
    ...
    if(z->type != LIBERTY__VAL_DOUBLE)
	{
		*err = SI2DR_INVALID_OBJECTTYPE;
		if( liberty___debug_mode )
		{
			...
			(*si2ErrMsg)(SI2DR_SEVERITY_ERR, SI2DR_INVALID_OBJECTTYPE, 
						 "si2drComplexValGetFloat64Value:", &err2);
		}
		return 0;
	}
	*err = SI2DR_NO_ERROR;
    ...
    if(z->type != LIBERTY__VAL_STRING)
	{
		*err = SI2DR_INVALID_OBJECTTYPE;
		if( liberty___debug_mode )
		{
			...
			(*si2ErrMsg)(SI2DR_SEVERITY_ERR, SI2DR_INVALID_OBJECTTYPE, 
						 "si2drComplexValGetStringValue:", &err2);
		}
		return 0;
	}
	*err = SI2DR_NO_ERROR;
...
{
	liberty_attribute_value *z = (liberty_attribute_value*)attr.v2;
    if(z->type != LIBERTY__VAL_BOOLEAN)
	{
		*err = SI2DR_INVALID_OBJECTTYPE;
		if( liberty___debug_mode )
		{
			...
			(*si2ErrMsg)(SI2DR_SEVERITY_ERR, SI2DR_INVALID_OBJECTTYPE, 
						 "si2drComplexValGetBooleanValue:", &err2);
		}
		return 0;
	}
	*err = SI2DR_NO_ERROR;
    ...
    if(z->type != LIBERTY__VAL_EXPR)
	{
		*err = SI2DR_INVALID_OBJECTTYPE;
		if( liberty___debug_mode )
		{
			...
			(*si2ErrMsg)(SI2DR_SEVERITY_ERR, SI2DR_INVALID_OBJECTTYPE, 
						 "si2drComplexValGetExprValue:", &err2);
		}
		return 0;
	}
	*err = SI2DR_NO_ERROR;
...
si2drAttrComplexValIdT   si2drIterNextComplex              ( si2drValuesIdT iter,
															 si2drErrorT     *err )
{
	si2drAttrComplexValIdT retoid, toid;
	iterat *y = (iterat*)iter;
	liberty_attribute_value *z;
	if( trace )
	{
	...
	retoid.v1 = (void*)SI2DR_COMPLEXVAL;
	retoid.v2 = (void*)z;
										...
										si2drErrorT  *err)
{
	...
	if( name && *name )
	{
		...
		if( toid.v1 != (void*)0 )
		{
			...
			else
			{
				...
				d->group_type = /* OLD: x NEW: */ liberty_strtable_enter_string(master_string_table,x);
				*err = SI2DR_NO_ERROR;
	...
	liberty_hash_enter_oid(g->define_hash, d->name, retoid);      /* fix via chrisj's posting: d->name instead of using name */
	liberty_hash_enter_oid(master_define_hash, d->name, retoid);  /* nanda, btw, also spotted and fixed this */
								...
								si2drErrorT *err)
{
	...
	if( gt == gm ) /* Thanks to Chenzhong Luo of Synopsys for discovering the necessity for this and the next check */ 
	{
							   ...
							   si2drErrorT *err)
{
	...
	*err = SI2DR_NO_ERROR;
	...
	if( gt == gm )
		return;
	if( gt->next == gm ) /* already done. Thanks to Chenzhong Luo for finding a bug here! */
		return;
