...
{
    context->weights.lseW = context->hd->cfg.ValueOf("GlobalPlacement.Weights.alphaTWL", 1.0);
    ...
    double sprWInitialMultiplier = context->hd->cfg.ValueOf("GlobalPlacement.Weights.spreadingWeightInitialMultiplier", 0.1);
...
{
    context.weights.sprW *= hd.cfg.ValueOf("GlobalPlacement.Weights.muSpreadingMultiplier", 2.0);

    if (context.useLR)
    {
        context.LRdata.UpdateMultipliers(hd);
        context.weights.lseW *= hd.cfg.ValueOf("GlobalPlacement.Weights.alphaTWLMultiplier", 1.0);
    }
}
