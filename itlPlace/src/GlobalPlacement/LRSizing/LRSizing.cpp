...
void LRSizer::GetStPEdges()
{
  ...
  double length=0;
  for (HTimingPointWrapper pt = design[design.TimingPoints.TopologicalOrderRoot()]; !::IsNull(pt.GoPrevious()); )
  {
    ...
    if ((pin.IsPrimaryInput())||(pin.Direction()==PinDirection_OUTPUT))
    {
      ...
      for(std::vector<StPEdge>::iterator iter = StPEdges.begin();iter!=StPEdges.end();iter++)
      {
        ...
        fprintf (nets, "%lf;%lf;%lf;%lf;%lf;%lf;%lf\n",iter->startX,iter->startY,iter->endX,iter->endY,length,length*design.RoutingLayers.Physics.RPerDist,length*design.RoutingLayers.Physics.LinearC);
...
void LRSizer::GetMacroTypeCharacteristics()
{
      ...
      else if(pinE.Direction() == PinDirection_OUTPUT)
      {
        for(HPinType::ArcsEnumeratorW arcE = pinE.GetArcsEnumeratorW(); arcE.MoveNext();)
        {
          ...
          fprintf(macroCharacteristics, "%s;%lf;%s;Output; ;%s;%lf;%lf\n", 
            cellName.data(), cellSize, pinName.data(), signalDirection.data(), arcE.ResistanceRise(), arcE.TIntrinsicRise());
        }
      }
    }
  }
...
void LRSizer::GetPinsCharacteristics()
{
...
//  double c, r,t;
  ...
  for (HTimingPointWrapper tp = design[design.TimingPoints.TopologicalOrderRoot()].GoNext(); !::IsNull(tp); tp.GoNext())
  {
    if(!::IsNull(tp))
    {
      ...
      fprintf (pinsCharacteristics, "%s;%s;%d;%s;%lf;%lf;%lf;%lf\n",cellName.data(),pinName.data(),::ToID(tp),direction.data(),tp.ArrivalTime(),DPhisics.C, DPhisics.R, DPhisics.T);
    }
  }
...
void LRSizer::ApplySizing(std::vector<double>& X)
{
  ...
  while(cellFrom!=cells->end() && currentX!=X.end())
  {
    ...
    //DEBUG
    double roundingToSize=getMacroTypeSize(cellToType);//design.GetDouble<HMacroType::SizeX>(cellToType);
    ...
    if (cellFromType != cellToType)
    {
      ...
      double fromWidth=getMacroTypeSize(cellFromType);//design.GetDouble<HMacroType::SizeX>(cellFromType);
      double toWidth=getMacroTypeSize(cellToType);//design.GetDouble<HMacroType::SizeX>(cellToType);
...
void LRSizer::getCellFamily(HCell cell, std::vector<double>& cellSizes)
{
  ...
  {
    double size=getMacroTypeSize(*macroType);//design.MacroTypes.GetDouble<HMacroType::SizeX>(*macroType);
        ...
        for (HMacroType::PinsEnumeratorW pinE = macroTypeE.GetEnumeratorW(); pinE.MoveNext();)
        {
          if ((pinE.Direction()==PinDirection_INPUT)||(pinE.Direction()==PinDirection_OUTPUT))
          {
            if(pinE.Name() == pinName)
            {
              ...
              X.push_back(getMacroTypeSize(macroTypeE)/*macroTypeE.SizeX()*/); 
              break;
            }
          }
        }
...
LRSizer::LRSizer(HDesign& hd): design(hd), LambdaIn(0)
{
  ...
  cells = InitCells();
  //DEBUG
  /* ALERT("Cells number %d", cells->size());
  for (std::vector<HCell>::iterator iter=cells->begin(); iter!=cells->end();iter++)
  {
  ALERT("cells: %s", (design.MacroTypes.GetString<HMacroType::Name>((*iter,design).Type())).data());
  }*/
  //END OF DEBUG
  ...
  //ALERT("RegressedR: %lf", regressionR->getValue(1/7));
  ...
  //DEBUG
  GetPinsCharacteristics();
  //GetStPEdges();
  //std::string macroTypeName="string";
  //printf("%d\t%c",macroTypeName.length(), macroTypeName[5]);

  GetMacroTypeCharacteristics();
...
double LRSizer::CalcB(HCell& cell,std::vector<double>& vMu,std::vector<double>& vX )
{
  ...
  for (HCell::PinsEnumeratorW pin = (design, cell).GetPinsEnumeratorW(); pin.MoveNext(); )
  {
    if (pin.Direction() == PinDirection_OUTPUT && !::IsNull(pin.Net()))
    {
      ...
      B=B+vMu[::ToID(tp)]*GetObservedC(tp,vX)*(regressionR->getValue(1/vX[index]))*vX[index];
    }
  }
  return B;
}
...
void LRSizer::initVX( std::vector<double>& vX )
{
  ...
  for(std::vector<HCell>::iterator iter=cells->begin();iter!=cells->end();iter++)
  {
    HMacroType cellType=(*iter,design).Type();
    vX.push_back(getMacroTypeSize(cellType));
  }
  ...
  if (pin.Direction() == PinDirection_OUTPUT && !::IsNull(net))
  {
    ...
    for(HCell::PinsEnumeratorW inputPin = cell.GetPinsEnumeratorW(); inputPin.MoveNext(); )
      if (inputPin.Direction() == PinDirection_INPUT && !::IsNull(inputPin.Net()))
      {
        ...
        int index = (int)distance(cells->begin(), find(cells->begin(), cells->end(), cell));
        sum=sum+vMu[::ToID(design.TimingPoints[inputPin])]*(regressionR->getValue(1/vX[index]));
...
double LRSizer::TestCalculateInputTPDelay(std::vector<double>& vX,HTimingPoint& tp,std::vector<double>& arrivalTimes)
{
...
double LRSizer::TestCalculateOutputTPDelay(std::vector<double>& vX,HTimingPoint& tp,std::vector<double>& arrivalTimes)
{
...
double LRSizer::CalculateQ(unsigned int size)
{
  ...
  for (; tp!=design.TimingPoints.FirstInternalPoint(); tp.GoPrevious())
  {
    ...
    int index = (int)distance(cells->begin(), find(cells->begin(), cells->end(), cell));
    SumAllInputLambda+=SumInputLambdaForPoint*(regressionR->getValue(1/XLower[index])*GetObservedC(tp,XLower));
...
bool LRSizer::CheckStopConditionForLDP(std::vector<double>& vX, double errorBound)
{
  ...
  for(unsigned int i=0; i < vX.size(); i++)
    Sum+=vX[i];
  if ((Sum-CalculateQ((int)vX.size()))<errorBound) return true;
  else false;
}
double LRSizer::CalcCriterion(std::vector<double>& vCurrentX)
{
  ...
  for (; tp!=design.TimingPoints.FirstInternalPoint(); tp.GoPrevious())
  {
    ...
    Maths::Regression::Linear* regressionR = getRegressionR(tp);
    int index = (int)distance(cells->begin(), find(cells->begin(), cells->end(), cell));
    SumAllInputLambda+=SumInputLambdaForPoint*(regressionR->getValue(1/vCurrentX[index])*GetObservedC(tp,vCurrentX));
  }
...
HMacroType LRSizer::roundCellToTypeFromLib(HCell cellFrom, double currentX)
{
  ...
  for(std::vector<HMacroType>::iterator macroType = macroTypesInFamily.begin(); macroType != macroTypesInFamily.end(); macroType++)
  {
    double sizeOfMacroType=getMacroTypeSize(*macroType);//design.MacroTypes.GetDouble<HMacroType::SizeX>(*macroType);
