...
void lefiLayer::parseSpacing(int index) {
   ...
   char   *value;
   double spValue, width, within, cutArea;
   char   msg[1024];
   int    numCuts;
      ...
      if ((strcmp(value, "CENTERTOCENTER") == 0) ||
         ...
         if (strcmp(value, "LAYER") == 0) {
            ...
            if (strcmp(value, "WITHIN") == 0) {
               ...
               value = strtok(NULL, " ");
               if (*value != ';') {
                  sprintf(msg, "ERROR (LEFPARS-1321): Incorrect syntax defined for property LEF57_SPACING: %s.\nCorrect syntax is \"SPACING minSpacing ADJACENTCUTS {2|3|4} WITHIN cutWithin ;\"",
                  this->values_[index]);
                  lefiError(msg);
                  return;
               } else {
            ...
            value = strtok(NULL, " ");
            if (*value != ';') {
               sprintf(msg, "ERROR (LEFPARS-1322): Incorrect syntax defined for property LEF57_SPACING: %s.\nCorrect syntax is \"SPACING minSpacing PARALLELOVERLAP ;\"",
               this->values_[index]);
               lefiError(msg);
               return;
            } else {
            ...
            value = strtok(NULL, " ");
            if (*value != ';') {
               sprintf(msg, "ERROR (LEFPARS-1323): Incorrect syntax defined for property LEF57_SPACING: %s.\nCorrect syntax is \"SPACING minSpacing AREA cutArea ;\"",
               this->values_[index]);
               lefiError(msg);
               return;
            } else {
...
void lefiLayer::parseMinstep(int index) {
   char   *wrkingStr = strdup(this->values_[index]);
   char   *value;
   double minStepLength, maxLength;
   int    maxEdges;
   char   msg[1024];
