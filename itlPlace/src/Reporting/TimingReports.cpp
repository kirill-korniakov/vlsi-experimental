...
void ReportCriticalPathsToLogFile(HDesign& hd, int nPaths)
{
  ...
  for(int k = 0; k < nPaths && k < countCP; ++k)
    PrintPath(hd, hd[vec[k].path], k + 1);
...
void PrintPath(HDesign& design, HCriticalPath path, int pathNumber)
{
  ...
  HCriticalPath::PointsEnumeratorW cpoint = design.Get<HCriticalPath::Points,HCriticalPath::PointsEnumeratorW>(path);
  HCriticalPathPointWrapper previousSink = design[design.CriticalPathPoints.Null()];
  while (cpoint.MoveNext())
  {
    ...
    HPinWrapper driverPin = design[design[driver.TimingPoint()].Pin()];
    HSteinerPointWrapper stPointDriver = design[design.SteinerPoints[driverPin]];
    HSteinerPointWrapper stPointSink = design[design.SteinerPoints[design[cpoint.TimingPoint()].Pin()]];
    ...
    tf.SetCell(ColCellName, driverPin.IsPrimaryInput() ? driverPin.Name() : design[driverPin.Cell()].Name());
...
void PrintCriticalPaths(HDesign& hd, int nPaths)
{
  ...
  if (min(nPaths, countCP) > 0)
    WRITELINE("");
  ALERTFORMAT(("Reported %d paths of %d", min(nPaths, countCP), countCP));
  ALERT("Critical paths printed!");  
}