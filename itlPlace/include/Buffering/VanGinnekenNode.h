...
// o
//
// "O" - сурс, "o" - синки, "0" - внутреняя точка штейнерского дерева, "x" - виртуальное звено, 
// "|" - переход(ребро) влево, " - " - переход(ребро) в право(необязательно существует)
class VGNode
{
public:
  ...
  //NOTE: является сурсом, внутреним, синком
  bool IsSource() const ;
...
protected:
  ...
  HSteinerPoint m_LeftEnd; //NOTE: левая вершина поддерева
  HSteinerPoint m_RightEnd; //NOTE: правая вершина поддерева

  int           m_steps;//NOTE:количество точек на отрезке от начала до конца
  int           m_indexStart;//NOTE: номер в сквозной нумерации
  int           m_RelativePosition; //текущая позиция 0 - левый, 1 - правый, 2 - сурс (или начало)
