...
void estimateOneSeg(Segment *seg)
{
    ...
    // assign 0.5 to both Ls (x1,y1)-(x1,y2) + (x1,y2)-(x2,y2) + (x1,y1)-(x2,y1) + (x2,y1)-(x2,y2)
...
void routeSegL(Segment *seg)
{
    ...
    else // L route
    {
        ...
        {
             grid = i*xGrid;
             tmp = v_edges[grid+seg->x1].red + v_edges[grid+seg->x1].est_usage-vCapacity_lb;
             if(tmp>0)
                 costL1 += tmp;
             tmp = v_edges[grid+seg->x2].red + v_edges[grid+seg->x2].est_usage-vCapacity_lb;
             if(tmp>0)
                 costL2 += tmp;
        }
        grid = seg->y2*(xGrid-1);
        ...
        for(i=seg->x1; i<seg->x2; i++)
        {
             ...
             if(tmp>0)
                 costL1 += tmp;
             tmp = h_edges[grid1+i].red + h_edges[grid1+i].est_usage-hCapacity_lb;
             if(tmp>0)
                 costL2 += tmp;
        }
...
// route all segments with L, firstTime: TRUE, no previous route, FALSE - previous is L-route
...
void newrouteL(int netID, RouteType ripuptype, Bool viaGuided)
{
    ...
    for(i=0; i<2*d-3; i++)
    {
        if(sttrees[netID].edges[i].len>0) // only route the non-degraded edges (len>0)
        {
            ...
            else // L-routing
            {
                ...
                {
                    grid = j*xGrid;
                    tmp = v_edges[grid+x1].est_usage-vCapacity_lb + v_edges[grid+x1].red;
                    if(tmp>0)
                        costL1 += tmp;
                    tmp = v_edges[grid+x2].est_usage-vCapacity_lb + v_edges[grid+x2].red;
                    if(tmp>0)
                        costL2 += tmp; 
                }
                grid = y2*(xGrid-1);
                ...
                for(j=x1; j<x2; j++)
                {
                    ...
                    if(tmp>0)
                        costL1 += tmp;
                    tmp = h_edges[grid1+j].est_usage-hCapacity_lb + h_edges[grid1+j].red;
                    if(tmp>0)
                        costL2 += tmp;
                }
...
void newrouteZ_edge(int netID, int edgeID)
{
    ...
    if(sttrees[netID].edges[edgeID].len>0) // only route the non-degraded edges (len>0)
    {
        ...
        if(x1!=x2 || y1!=y2) // not H or V edge, do Z-routing (if H or V edge, no need to reroute)
        {
            // ripup the original routing
            newRipup(treeedge,treenodes, x1, y1, x2, y2);
            ...
            for(i=x1; i<=x2; i++)
            {
                ...
                for(j=ymin; j<ymax; j++)
                {
                    tmp = v_edges[grid+i].est_usage-vCapacity_lb + v_edges[grid+i].red;
                    grid += xGrid;
					if(tmp>0) {
                        costV[i-x1] += tmp;
            ...
            {
                costTB[i] = costTB[i-1];
                tmp = h_edges[grid1+i-1].est_usage-hCapacity_lb + h_edges[grid1+i-1].red; 
				if(tmp>0) {
                    costTB[i] += tmp;
					costTBtest[i] += HCOST;
				} else {
                ...
                tmp = h_edges[grid2+i-1].est_usage-hCapacity_lb + h_edges[grid2+i-1].red;
				if(tmp>0) {
                    costTB[i] -= tmp;
					costTBtest[i] -= HCOST;
					} else {
            ...
            if(x1!=x2 && y1!=y2) // not H or V edge, do Z-routing
            {
				...
				if (newRipupType2(treeedge,treenodes, x1, y1, x2, y2, d)) {
					...
					if(y1Smaller)
					{
						for(j=y1; j<y2; j++)
						{
							tmp = v_edges[j*xGrid+x2].est_usage-vCapacity_lb + v_edges[j*xGrid+x2].red;
							if(tmp>0)
								costLR[0] += tmp;
						}
...
void routeMonotonic(int netID, int edgeID, int threshold)
{
    ...
    if(sttrees[netID].edges[edgeID].route.routelen>threshold) // only route the non-degraded edges (len>0)
    {
        ...
        if(x1!=x2 || y1!=y2) // not H or V edge, do Z-routing (if H or V edge, no need to reroute)
        {
            ...
            treeedge->route.gridsX = (int*)realloc(treeedge->route.gridsX, cnt*sizeof(int));
            treeedge->route.gridsY = (int*)realloc(treeedge->route.gridsY, cnt*sizeof(int));
            if(x1!=gridsX[0] || y1!=gridsY[0]) // gridsX[] and gridsY[] store the path from n2 to n1
            {
                cnt = 0;
    ...
    if(treeedge->len>0) // only route the non-degraded edges (len>0)
    {
        ...
        else // L-routing
        {
            ...
            for(j=ymin; j<ymax; j++)
            {
                ...
                if(tmp>0)
                    costL2 += tmp; 
				//costL1 += simpleCost (v_edges[grid+x1].est_usage, v_edges[grid+x1].cap);
				//costL2 += simpleCost ( v_edges[grid+x2].est_usage,  v_edges[grid+x2].cap);
            }
            grid = y2*(xGrid-1);
            grid1 = y1*(xGrid-1);
            for(j=x1; j<x2; j++)
            {
                ...
                if(tmp>0)
                    costL2 += tmp;
				//costL1 += simpleCost (h_edges[grid+j].est_usage, h_edges[grid+j].cap);
				//costL2 += simpleCost (h_edges[grid1+j].est_usage, h_edges[grid1+j].cap);
...
void routeLVEnew(int netID, int edgeID, int threshold, int enlarge)
{
    ...
    if(sttrees[netID].edges[edgeID].len>threshold) // only route the non-degraded edges (len>0)
    {
		...
		if ( newRipupCheck(treeedge, x1, y1, x2, y2,threshold, netID, edgeID)) {
			...
			treeedge->route.routelen = cnt-1;
			treeedge->route.gridsX = (int*)calloc(cnt, sizeof(int));
			treeedge->route.gridsY = (int*)calloc(cnt, sizeof(int));
...
void routeLVAll(int threshold, int expand)
{
	...
	for(i=0; i<forange; i++) {
		h_costTable[i] = 8/(exp((float)(hCapacity-i)*1.33)+1) + 1 ;// /hCapacity*30));
    ...
    for(i=0; i<2*d-3; i++)
    {
        if(sttrees[netID].edges[i].len>0) // only route the non-degraded edges (len>0)
        {
            ...
            else // L-routing
            {
                ...
                {
                    grid = j*xGrid;
                    tmp = v_edges[grid+x1].usage-vCapacity_lb + v_edges[grid+x1].red;
                    if(tmp>0)
                        costL1 += tmp;
                    tmp = v_edges[grid+x2].usage-vCapacity_lb + v_edges[grid+x2].red;
                    if(tmp>0)
                        costL2 += tmp; 
					//costL1 += simpleCost (v_edges[grid+x1].est_usage, v_edges[grid+x1].cap);
					//costL2 += simpleCost ( v_edges[grid+x2].est_usage,  v_edges[grid+x2].cap);
                ...
                for(j=x1; j<x2; j++)
                {
                    ...
                    if(tmp>0)
                        costL1 += tmp;
                    tmp = h_edges[grid1+j].usage-hCapacity_lb + h_edges[grid1+j].red;
                    if(tmp>0)
                        costL2 += tmp;
                }
