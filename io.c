...
Node *Input()
{
  ...
  while( (c = fgetc(fp)) != EOF) {
	...
	if ( !strcmp(buf2, "driv") )  {	 
	  ...
	  nodes[n_nodes].left = (Node *)NULL;
	  nodes[n_nodes].right = (Node *)NULL;
	  n_sinks ++;
	  n_nodes ++;
	}
	if ( !strcmp(buf2, "sink") ) {
  ...
  printf("Wire Segmenting coef (0 means no segmenting:) ");
  scanf("%d", &N_SEG);  
  if (N_SEG!=0)
    wire_segment(N_SEG);
  return t;
...
Node *produce_tree()
{
  ...
  for ( i=0;i<n_sinks+n_candidates;i++ ) {
    ...
    if ( target[1] > -1 )
      ...
      nodes[i].right = (Node *)NULL;		
  }
  return &nodes[0];
}

/* Print tree on screen */
void ntree_print( p, ind )
     Node *p;
     int ind;
{
