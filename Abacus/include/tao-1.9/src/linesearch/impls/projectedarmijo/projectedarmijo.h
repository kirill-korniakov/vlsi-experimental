...
// minimization.
//
// Given a function f, the current iterate x, and a projected descent 
// direction d:
// Find the smallest i in 0, 1, 2, ..., such that:
//
//    f(P(x + (beta**i)d)) <= 
//        f(x) - (sigma)<grad f(x),x - P(x + (beta**i)d)>
//
...
//
//    f(P(x + (beta**i)d)) <= 
//        R    - (sigma)<grad f(x),x - P(x + (beta**i)d)>
//
...
#ifndef __TAO_PROJECTEDARMIJO_H
#define __TAO_PROJECTEDARMIJO_H
