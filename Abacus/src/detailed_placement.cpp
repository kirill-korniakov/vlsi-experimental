...
//----------------------------------------------------------------------------------
void CellsInCluster::AddCellToCluster(const Circuit &circuit, const RowElement &_element, bool recalc) {
		cells_i_cl[curnum++] = _element;
		
		if( recalc ) {
			weight += circuit.nodes[_element.cellIdx].width * circuit.nodes[_element.cellIdx].height; //circuit.weights[_element.cellIdx].wts; // TODO: check
			//q += circuit.weights[_element.cellIdx].wts * (_element.xCoord - width); //????????????????????????
			q += (_element.xCoord - width) * circuit.nodes[_element.cellIdx].width * circuit.nodes[_element.cellIdx].height; 
			width += circuit.nodes[_element.cellIdx].width;

			if( width > circuit.width) {
				cout<<"AddCellToCluster: width of cluster > width of circuit!\n";
				system("PAUSE");
			}
		}
}

void CellsInCluster::free_cluster() {
	if( cells_i_cl )
		delete [] cells_i_cl;
	
	cells_i_cl = NULL;
	curnum = 0;
	weight = 0; width = 0; q = 0; x = 0;
}

CellsInCluster::~CellsInCluster() {
	if( cells_i_cl )
		delete [] cells_i_cl;
	weight = 0; width = 0; q = 0; x = 0;
}

void CellsInRow::AddCellToRow(Circuit &circuit, int cellind) {
	RowElement temp;
	temp.xCoord = circuit.placement[cellind].xCoord - circuit.nodes[cellind].width * 0.5;
	temp.cellIdx = cellind;
	cells_ir.push_back(temp);
	cellsnum++;
}

void CellsInRow::AddCellToRow( const Circuit &circuit, const RowElement &_element) {
	RowElement temp = _element;

	/*if(temp.xCoord < 0)
		temp.xCoord = 0;

	if(temp.xCoord + circuit.nodes[temp.cellIdx].width > circuit.width)
		temp.xCoord = circuit.width - circuit.nodes[temp.cellIdx].width;*/

	cells_ir.push_back(temp);
	cellsnum++;
}

void CellsInRow::AddCluster(const Circuit &circuit, int first, int second) {
	for(int i = 0; i < clusters[second].curnum; i++)										//add all cells from second
		clusters[first].AddCellToCluster(circuit, clusters[second].cells_i_cl[i], false);	//cluster to first

	clusters[first].weight += clusters[second].weight;	//recalculate weight, q and width
	clusters[first].q += clusters[second].q - clusters[second].weight * clusters[first].width;
	clusters[first].width += clusters[second].width;

	if( clusters[first].width > circuit.width ) {
		cout<<"AddCluster: width of cluster "<<first<<" > width of circuit!\n";
		system("PAUSE");
	}
}

void CellsInRow::RemoveLastCluster() {
	clusters[clustersnum - 1].free_cluster();
	clustersnum--;
}

void CellsInRow::Collapse(const Circuit &circuit, int ind) {
	double _x = clusters[ind].q / clusters[ind].weight;
	double x_min = 0;
	double x_max = circuit.width;
	//cout<<"x_max= "<<x_max<<endl;

	if( _x < x_min ) //x_min, x_max???????????????????????????? -- check it
		_x = x_min;

	if(_x > x_max - clusters[ind].width )
		_x = x_max - clusters[ind].width;
	
	clusters[ind].x = _x;	
	//cout<<"Collapse: _x= "<<_x<<"; cluster's width= "<<clusters[ind].width<<"; ind of cluster= "<<ind<<endl;
	int ind2 = ind - 1;

	if(ind2 >= 0) { //previous cluster exists
		if(clusters[ind2].x + clusters[ind2].width > clusters[ind].x) { // last cluster overflows with previous?
			AddCluster(circuit, ind2, ind);										//then merge them
			RemoveLastCluster();
			Collapse(circuit, ind2);											//check next cluster
		}
	}
}

bool CellsInRow::PlaceRow(const Circuit &circuit, Coordinates coordinates[]) { 
	clusters = new CellsInCluster[cells_ir.size()];
	clustersnum = 0;
	double totalWidth = 0;

	for(std::list<RowElement>::const_iterator i = cells_ir.begin(); i != cells_ir.end(); i++){ //for all cells in row
		int c = clustersnum - 1; //c = last cluster
		totalWidth += circuit.nodes[i->cellIdx].width;

		if( totalWidth > circuit.width ) {
			cout<<"PlaceRow: totalWidth of cells in row > width of circuit: tw= "<<totalWidth<<endl;
			system("PAUSE");
			delete [] clusters;
			clustersnum = 0;
			return false;
		...
		if( ( c < 0 ) || (clusters[c].x + clusters[c].width <= i->xCoord) ) { //first element or no overflows
			c = clustersnum;
			clusters[c].x = i->xCoord; //create new cluster
			clusters[c].q = 0;
			clusters[c].width = 0;
			clusters[c].weight = 0;
			clusters[c].create_array(cells_ir.size() );
			clusters[c].AddCellToCluster(circuit, *i, true);
			clustersnum++;
		}
	...
	for(int j = 0; j < clustersnum; j++) { //all clusters
		double _x = clusters[j].x; //TODO: check it...

		if(_x < 0) {
			cout<<"PlaceRow: _x= "<<_x<<"; ind of cluster= "<<j<<endl;
			system("PAUSE");
		}

		for(int l = 0; l < clusters[j].curnum; l++) { //all cells in this cluster
			int idx = clusters[j].cells_i_cl[l].cellIdx;
			coordinates[idx].x = _x; //new coord
			_x += circuit.nodes[idx].width;
		}
	}
...
MULTIPLACER_ERROR Abacus_Legalize(Circuit& circuit) {
  int NUM_OF_ROWS_IN_LEGALIZATION = circuit.nRows; // parameter for Legalization
  cout<<"legalization started\n";

  //TODO: improve num of rows --

  // Array of cells sorted by their x-coord
  RowElement *sortedCells;
  CellsInRow *cir = new CellsInRow [circuit.nRows];
  Coordinates *coordinates = new Coordinates [circuit.nNodes];
  //double *rightBordersOfRows;
  double *rowLengths;
  double circuitDiagonal = sqrt(circuit.height * circuit.height + circuit.width * circuit.width);
  double currX = 0.0;
  ...
  double bestX = 0.0;
  double bestY = 0.0;
  double width = 0.0;  //width of current cell
  double distance;
  double minDistance = 0.0;
  int currCell;
  int newRowIdx = 0;
  ...
  {
    sortedCells[i].cellIdx = i;
    double _x  = circuit.placement[i].xCoord - circuit.nodes[i].width * 0.5; //left corner of a cell
	  sortedCells[i].xCoord = _x;
  } 

  QuickSortRowElement(sortedCells, circuit.nNodes - 1);

  for (int i = 0; i < circuit.nNodes; i++) {
      double _x = sortedCells[i].xCoord;

      if(_x < 0)
          _x = 0;

      if(_x + circuit.nodes[i].width > circuit.width)
          _x = circuit.width - circuit.nodes[i].width;
  ...
  for (i = 0; i < circuit.nNodes; ++i)
  {
	  int bestRow = 0; //the best row for this cell
	  currCell  = sortedCells[i].cellIdx;    
    width = circuit.nodes[currCell].width;
	  currX     = sortedCells[i].xCoord; //circuit.placement[currCell].xCoord; //- fmod(circuit.placement[currCell].xCoord - halfWidth, siteWidth);
    currY     = circuit.placement[currCell].yCoord - siteHeight * 0.5; //down corner of a cell
    currRowIdx  = static_cast<int>( floor(currY / siteHeight) );
	  bestRow = currRowIdx;	
    //firstRowIdx = max(currRowIdx - NUM_OF_ROWS_IN_LEGALIZATION, 0);
    //lastRowIdx  = min(currRowIdx + NUM_OF_ROWS_IN_LEGALIZATION, circuit.nRows - 1);
    minDistance = circuitDiagonal;
	  ...
	  for (j = 0; j < circuit.nRows; ++j)
    {
	    /*if( (j < currRowIdx) && ( (currRowIdx - j) * siteHeight >= minDistance ) ) { //TODO: check it
		    cout<<"lowbound > mindistance1\n";
		    continue;
	    }

	    if( (j > currRowIdx) && ( (j - currRowIdx) * siteHeight >= minDistance ) ) {
		    //cout<<"lowbound > mindistance2\n";
		    break;
	    }*/

      if ( rowLengths[j] + width > circuit.width )
        continue;

	    cir[j].AddCellToRow(circuit, sortedCells[i]);
	    bool placed = cir[j].PlaceRow(circuit, coordinates);
	    cir[j].cells_ir.pop_back();
	    cir[j].cellsnum--;

	    if( placed == false ) {
		    cout<<"row "<<j<<": PlaceRow failed...\n";
		    continue;
	    }

	    newX = coordinates[currCell].x;      
	    newY = j * siteHeight;
      distance = sqrt((newX - currX) * (newX - currX) + (newY - currY) * (newY - currY));
    ...
    cir[bestRow].AddCellToRow(circuit, sortedCells[i]);
	  cir[bestRow].PlaceRow(circuit, coordinates);
    coordinates[currCell].y = bestY;	  
    //rightBordersOfRows[newRowIdx] = bestX + halfWidth;
    rowLengths[bestRow] += width;
  }

  for( i = 0; i < circuit.nNodes; i++) { //TODO: check it
		circuit.placement[i].xCoord = coordinates[i].x  - fmod(coordinates[i].x, siteWidth) + circuit.nodes[i].width * 0.5;
		circuit.placement[i].yCoord = coordinates[i].y + siteHeight * 0.5;
  }
