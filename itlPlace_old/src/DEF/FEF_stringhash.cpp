 ...
 *   table = STR_make_stringtab(store);
 *   bool = STR_present_in_stringtab(table, string, &val) 
 *   insert(table, string, ptr);
 */
/* 8-20-1999 - Wanda da Rosa
 *   Added include <strings.h> for IBM build
 */
...
STR_make_stringtab(enum table_alloc we_alloc, const char *comment,
	...
	if(! table_names[i]) defrNerr(37);
	strcpy(table_names[i],comment);
	}
    return htab;
}

int STR_present_in_stringtab(void *hdl,const char *str,int **ret)
{
...
int STR_modify_stringtab(void *hdl,const char *str,void *new_val)
{
    HTABLE *htab = (HTABLE *)(hdl);
    int h = (htab->hash_fn)(str) & (htab->mask);
    int r; 
...
//                    the preprocessor, changes const char* to char*
char *STR_make_string_in_stringtab(HTABLE* htab,const char *str,void *val)
{
    int h;
...
void STR_add_to_stringtab(void *hdl, const char *str, void *val)
{
    HTABLE *htab = (HTABLE *)(hdl);
    int h;
...
void STR_destroy_stringtab(void *handle)
{
...
int STR_active_hashtables(const char *indent)
{
int i;
int count=0;
char str[100];
...
const char* defAliasKey(int index) {
...
const char* defAliasData(int index) {
  const char* retValue;
  HTABLE* h = (HTABLE*)def_alias_set;
  if (h == 0) return 0;
...
int defAddAlias(const char* key, const char* data) {
  HTABLE* h = (HTABLE*)def_alias_set;
  if (def_alias_set == 0) {
