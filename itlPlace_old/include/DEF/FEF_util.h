...
/* utility to strip color change codes from UI_message() */
const char *strip_color_codes(char *msg);

/* utility distance routine */
int UTIL_point_inside_box(int xll,int yll,int xur,int yur
			,int x,int y);
double UTIL_point_to_box(int xll,int yll,int xur,int yur
			,int x,int y);
int UTIL_box_intersects_box(int x1,int y1,int x2,int y2
			   ,int xx1,int yy1,int xx2,int yy2);
int UTIL_rect_is_smaller(int Rxll,int Ryll,int Rxur,int Ryur
		        ,int R2xll,int R2yll,int R2xur,int R2yur);
int UTIL_rect_inside_rect(int Axll,int Ayll,int Axur,int Ayur
                          ,int Bxll,int Byll,int Bxur,int Byur);
void UTIL_rect_overlap(int rxll,int ryll,int rxur,int ryur
                            ,int r2xll,int r2yll,int r2xur,int r2yur
                            ,int *oxll,int *oyll,int *oxur,int *oyur);
float UTIL_rect_area(int rxll,int ryll,int rxur,int ryur);
void UTIL_get_bbox(int x1,int y1,int x2,int y2,int width
	,int *bx1,int *by1,int *bx2,int *by2);
void UTIL_point_to_rect_edge(int Rxll,int Ryll,int Rxur,int Ryur
        ,int x,int y
        ,int *mindist,int *xx,int *yy);
...
/* class for doing regular expression matching
 *  a regular expression can contain '*' instead of sequence of chars
 *  constructor takes the expression the use match() where needed
 *
 *class RegularExpression
 *	{
 *	public:
 *		char *expression; // the expression
 *		char *test;		  // the string being tested
 *		char case_sensitive;
 *	private:
 *		int recursive_test(int psn_expr,int psn_test);
 *	public:
 *		RegularExpression(char *given,int match_case=0);
 *		~RegularExpression();
 *		int NotExpression();
 *		int match(char *test);
 *	};
