  ...
  while(true)
  {
    while(cur)
    {
      ...
      if(cur->type == NODE_TYPE_SINK)
        fprintf(output,"(%8.2f; %8.2f) SINK %f %c\n",
        cur->x,cur->y,cur->NetPathDelay,cur->origin->chtype);
      else
        if(cur->origin != 0)
          fprintf(output,"(%8.2f; %8.2f) %f %c\n",
          cur->x,cur->y,cur->NetPathDelay,cur->origin->chtype);
        else
          fprintf(output,"(%8.2f; %8.2f) %f\n",
          cur->x,cur->y,cur->NetPathDelay);
