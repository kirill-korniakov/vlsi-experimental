...
SAWindow::SAWindow(int _nRows, int _first, double x_array[], int _num, const Circuit &circuit,
                   int indexes[]): RectangularWindow(_nRows, _first, x_array, _num, circuit,
                                                     indexes)
{
  T = MAX_T;
...
void SAWindow::ShiftCell(int cellInd, Circuit &circuit)
{
  ...
  if (cellInd == 0) //the first cell
  {
    ...
    while (currCoords.xCoord >= windowBounds[0])
    {
  ...
  if (cellInd == elementsNum) //the last cell
  {
    while (currCoords.xCoord <= windowBounds[nRows * 2 - 1]) ///?
    {
      elements[cellInd].XCoord = currCoords.xCoord;
      double newFuncValue = GetFuncValue(circuit);
...
{    
    ...
    for (int i = 0; i < nSteps; i++)
    {
        ...
        firstRowIdx = 1; //rand() % circuit.nRows;        
