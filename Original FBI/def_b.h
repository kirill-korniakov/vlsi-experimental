...
typedef struct rlnode {
	struct rlnode *next;
        struct rlnode *prev;
        struct rlnode *listnext;
        struct rlnode *listprev;
...
extern RLnode *cover();        /* function to find the concave cover of (q,c) list */
extern double P();

/* in tree.c */
extern RTnode * insert();
